...
const NvprofileStudent = () => {
  ...
  const fetchCv = async () => {
    try {
      ...
      const cvUrl = URL.createObjectURL(res.data);
      setCvUrl(cvUrl);
    } catch (err) {
  ...
  const fetchMotivationLetter = async () => {
    try {
      ...
      const letterUrl = URL.createObjectURL(res.data);
      setMotivationLetterUrl(letterUrl);
    } catch (err) {
      console.error("Error fetching motivation letter:", err);
  ...
  const fetchProfile = async () => {
    try {
      ...
      // Appeler les fonctions pour récupérer les médias
      await Promise.all([
        fetchProfilePicture(),
        fetchCoverPhoto(),
        fetchCv(),
  ...
  if (!profile) return <div className="no-profile">Aucun profil trouvé.</div>;
      ...
      <div style={{
        ...
        <div style={{ padding: "90px 40px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            {profile.headline && <p style={{ margin: "5px 0 0", color: "#666" }}>{profile.headline}</p>}
          </div>
          <div>
    ...
    Envoyer un message
            ...
            <button 
              ...
              style={{
                padding: "8px 16px",
                borderRadius: "20px",
                backgroundColor: showContacts ? "#e0e0e0" : "#f0f0f0",
          ...
          <div style={{ padding: "20px 40px" }}>
            ...
            <div style={{ display: "flex", gap: "20px", marginTop: "15px" }}>
              ...
              {motivationLetterUrl && (
                ...
                >
                  <span>📄</span> Lettre de motivation
                </a>
              )}
            </div>
          </div>
